...
use crate::space_adventure::Space;
...
impl World {
    ...
    fn populate_planet(&mut self, rng: &mut ChaCha8Rng, planet: &Planet) -> AppResult<()> {
        ...
        for _ in 0..number_free_pirates {
            self.generate_random_player(rng, Some(position), planet.id, base_level)?;
    ...
    pub fn generate_random_team(
        ...
        for position in 0..MAX_POSITION {
            let player_id =
                self.generate_random_player(rng, Some(position), home_planet_id, team_base_level)?;
            self.add_player_to_team(player_id, team_id)?;
        }

        loop {
            let player_id =
                self.generate_random_player(rng, None, home_planet_id, team_base_level)?;
            self.add_player_to_team(player_id, team_id)?;
    ...
    fn generate_random_player(
        ...
        let planet = self.get_planet_or_err(home_planet_id)?;
        let player = Player::random(rng, player_id, position, &planet, base_level);
    ...
    pub fn auto_assign_crew_roles(&mut self, player_ids: Vec<PlayerId>) -> AppResult<()> {
        ...
        log::info!("TEam bonus {:#?}", team_bonus);
    ...
    pub fn set_team_crew_role(&mut self, role: CrewRole, player_id: PlayerId) -> AppResult<()> {
        ...
        let mut team = if let Some(team_id) = player.team {
            self.get_team_or_err(team_id)?.clone()
        } else {
        ...
        if let Some(crew_player_id) = current_role_player {
            ...
            info!(
                "Removing {} from {}",
                current_role_player.info.shortened_name(),
                role
            );
    ...
    fn free_pirates_found_after_exploration(
        ...
        if amount > 0 {
            for _ in 0..amount {
                ...
                let player_id = self.generate_random_player(rng, None, planet.id, base_level)?;
    ...
    pub fn handle_tick_events(&mut self, current_tick: Tick) -> AppResult<Vec<UiCallback>> {
        ...
        if !is_simulating {
            // This check should not be necessary, but better safe than sorry.
            if let Some(space) = self.space.as_mut() {
                let deltatime =
                    (current_tick - self.last_tick_min_interval) as f64 / SECONDS as f64;
            ...
            TeamLocation::Exploring {
                ...
                if current_tick > started + duration {
                        ...
                        )
                    {
                        ...
                        return Ok(Some(UiCallback::PushUiPopup {
                            popup_message: PopupMessage::AsteroidNameDialog(current_tick),
                        }));
                    }
    ...
    fn tick_free_pirates(&mut self, current_tick: Tick) -> AppResult<UiCallback> {
        ...
        Ok(UiCallback::PushUiPopup {
            popup_message: PopupMessage::Ok("Free pirates refreshed".into(), false, current_tick),
        })
    }
    ...
    fn tick_players_update(&mut self) {
        for (_, player) in self.players.iter_mut() {
            ...
            // if player.team == Some(self.own_team_id) {
            ...
            for idx in 0..player.skills_training.len() {
                ...
                if idx > 15 && player.info.relative_age() > 0.15 {
                    continue;
                }

                // potential_modifier has a value ranging from 0.0 to 4.0.
                ...
                assert!(player.skills_training[idx] >= 0.0);
